#!/usr/bin/env bash
# Fixed clean installer: jalankan langsung di VPS, bukan script pembungkus.
set -Eeuo pipefail

# ============================================================
# KAHFITZY NEBULA PRO PTERODACTYL v9
# One command install: Pterodactyl Panel + Nebula FULL custom UI
# Support: Ubuntu 22.04/24.04, Debian 12/13
# v7: Login page & server manager FULL redesign — bukan sekedar CSS override
# ============================================================
...
banner() {
  clear
  echo -e "${CYAN}╔════════════════════════════════════════════╗${NC}"
  echo -e "${CYAN}   KAHFITZY NEBULA PRO v9 - ADMIN FIX UI  ${NC}"
  ...
  if confirm "Aktifkan SSL Let's Encrypt sekarang? Pastikan domain sudah mengarah ke IP VPS"; then
    ...
    if certbot --nginx -d "$domain" --non-interactive --agree-tos -m "$email" --redirect; then
      if [[ -f "$PANEL_DIR/.env" ]]; then
        set_env_value "$PANEL_DIR/.env" APP_URL "https://${domain}"
        cd "$PANEL_DIR" && php artisan config:clear || true
      fi
      systemctl reload nginx || true
      ok "SSL aktif. APP_URL diubah ke https://${domain}"
      return 0
    else
      warn "SSL gagal. Panel tetap jalan via HTTP. APP_URL tetap http://${domain}"
      systemctl reload nginx || true
      return 1
    fi
  else
    warn "SSL dilewati. Aktifkan nanti dengan: certbot --nginx -d $domain"
    warn "Setelah SSL aktif, ubah APP_URL di .env ke https://${domain} lalu jalankan: php artisan config:clear"
    return 1
  fi
}

# ============================================================
# NEBULA FULL CUSTOM THEME v8
...
/* ====================================================
   KAHFITZY NEBULA PRO v8 — Full custom theme
...
body.kn-has-sidebar #app.kn-app-shift {
...
/* KAHFITZY_NEBULA_ADMIN_FIX_V9_START */
/* KAHFITZY NEBULA v9 — Admin access fix */
.kn-nav-link.kn-admin-link {
  background: rgba(139,92,246,.12);
  border: 1px solid rgba(139,92,246,.22);
  color: #d8b4fe !important;
}
.kn-nav-link.kn-admin-link:hover,
.kn-nav-link.kn-admin-link.active {
...
.kn-admin-empty-card {
  ...
  padding: 1.4rem;
  border-radius: 22px;
  background: linear-gradient(160deg, rgba(10,16,42,.86), rgba(5,9,25,.78));
  border: 1px solid rgba(148,163,255,.16);
  box-shadow: 0 24px 70px rgba(0,0,0,.28), 0 0 40px rgba(139,92,246,.08);
  text-align: center;
  position: relative;
  z-index: 10;
}
...
.kn-admin-empty-title {
  font-family: var(--n-font-head, 'Inter', system-ui, sans-serif);
  font-size: 1.1rem;
  font-weight: 700;
  color: var(--n-text, #eef0ff);
  margin-bottom: .35rem;
}
.kn-admin-empty-sub {
  max-width: 430px;
  ...
  font-size: .82rem;
  line-height: 1.55;
  color: var(--n-muted, #8a9ac7);
}
.kn-admin-empty-actions {
  display: flex;
  align-items: center;
  justify-content: center;
  gap: .65rem;
...
@media (max-width: 768px) {
  .kn-nav-links { overflow-x: auto; scrollbar-width: none; }
  .kn-nav-links::-webkit-scrollbar { display: none; }
  .kn-nav-link { white-space: nowrap; }
  .kn-admin-empty-card { margin-top: 1rem; }
}

/* KAHFITZY_NEBULA_ADMIN_FIX_V9_END */
...
@media (max-width: 768px) {
  ...
  body.kn-has-sidebar #app.kn-app-shift { margin-left: 0 !important; }
...
/* ====================================================
   KAHFITZY NEBULA PRO v9 — Full UI injection
...
(function () {
  ...
  function isLoginForm() {
    ...
    const user = q('input[name="user"], input[name="email"], input[type="email"], input[type="text"]', form);
    const action = (form.getAttribute('action') || '').toLowerCase();
    const path = window.location.pathname.toLowerCase();
    return !!(pass && user && (path.includes('login') || path.includes('auth') || action.includes('login') || action.includes('auth') || path === '/'));
  }

  function getPage() {
    const p = window.location.pathname;
    if (isLoginForm())                                             return 'login';
    if (p === '/admin' || p.startsWith('/admin/'))                  return 'admin';
    if (p.match(/^\/server\/[^/]+$/))                              return 'server-home';
    if (p.match(/^\/server\/[^/]+\//))                             return 'server-detail';
    if (p === '/' || p === '/servers' || p === '/dashboard')        return 'servers';
    return 'other';
  }
  ...
  function injectTopbar(page) {
    ...
    // v9: topbar jangan menghilangkan akses admin.
    // Kalau route berubah via React, topbar lama di-refresh sekali.
    const oldTopbar = q('.kn-topbar');
    if (oldTopbar && oldTopbar.dataset.page === page) return;
    if (oldTopbar) oldTopbar.remove();

    const rawUserName = q('[data-user-name]')?.dataset?.userName ||
                        q('.navigation__avatar')?.textContent?.trim() ||
                        q('[class*="avatar"]')?.textContent?.trim() ||
                        'N';
    const userName = rawUserName.slice(0, 1).toUpperCase();
    ...
    bar.innerHTML = `
      ...
      <nav class="kn-nav-links">
        ...
        <a href="/admin" class="kn-nav-link kn-admin-link ${page === 'admin' ? 'active' : ''}">
          ${ICONS.admin} Admin
        </a>
        ${isServer ? `
  ...
  function injectLogin() {
    ...
    // Pterodactyl login bisa pakai name="username" / name="user" / email.
    // v8: selector dibuat luas agar field asli pasti ketemu.
    const emailInput    = q('input[name="username"], input[name="user"], input[name="email"], input[type="email"], input[autocomplete="username"], input[type="text"]', existingForm);
    const passwordInput = q('input[name="password"], input[type="password"], input[autocomplete="current-password"]', existingForm);
    ...
    // v8: React/Formik tidak cukup diisi pakai input.value saja.
    // Harus pakai native setter + dispatch input/change supaya state React ikut berubah.
    ...
    btn.addEventListener('click', () => {
      ...
      if (!emailInput || !passwordInput) {
        showLoginError('Login form asli tidak terbaca. Refresh halaman lalu coba lagi.');
        resetLoginButton();
        return;
      }
      ...
      // Isi form asli dengan cara yang aman untuk React/Formik.
      setReactValue(emailInput, user);
      setReactValue(passwordInput, pass);

      // Beri React 1 tick untuk update state, baru submit.
      setTimeout(() => {
        try {
          if (existingForm.requestSubmit) {
            existingForm.requestSubmit(submitBtn || undefined);
          } else if (submitBtn) {
            submitBtn.click();
          } else {
            const ev = new Event('submit', { bubbles: true, cancelable: true });
            const notCancelled = existingForm.dispatchEvent(ev);
            if (notCancelled) existingForm.submit();
          }
        } catch (e) {
          // Fallback terakhir.
          try {
            if (submitBtn) submitBtn.click();
            else existingForm.submit();
          } catch (err) {
            showLoginError('Gagal submit login: ' + (err && err.message ? err.message : err));
            resetLoginButton();
          }
        }
      }, 120);

      // Kalau login gagal, React biasanya render error di form asli yang sedang disembunyikan.
      // v8: jangan biarkan tombol stuck selamanya.
      setTimeout(() => {
        if (!q('.kn-login-wrap')) return;
        if (!window.location.pathname.toLowerCase().includes('login') && !window.location.pathname.toLowerCase().includes('auth')) return;

        const hiddenErrors = qq('[role="alert"], .text-red-500, .text-red-600, .text-red-700, [class*="error"], [class*="danger"]', existingForm)
          .map(e => (e.textContent || '').trim())
          .filter(Boolean)
          .slice(0, 3)
          .join(' ');

        showLoginError(hiddenErrors || 'Login belum berhasil. Cek username/password, lalu coba lagi.');
        resetLoginButton();
      }, 6500);
    });
  ...
  function upgradeServerCards() {
    // Cari server link card asli. Jangan ubah link di topbar/sidebar.
    const cards = qq('a[href*="/server/"]').filter(card => {
      if (card.closest('.kn-topbar, .kn-sidebar, nav, header, aside')) return false;
      if (card.classList.contains('kn-server-card') || card.dataset.knUpgraded) return false;
      try {
        const u = new URL(card.href, window.location.origin);
        return /^\/server\/[^/]+$/.test(u.pathname);
      } catch (_) {
        return false;
      }
    });
  ...
  function upgradeServerSidebar() {
    ...
    document.body.classList.add('kn-has-sidebar');
    q('#app')?.classList.add('kn-app-shift');
  ...
  /* ── v9: tampilkan akses Admin walau akun admin tidak punya server ── */
  function ensureAdminAccessCard() {
    if (q('.kn-admin-empty-card')) return;
    if (getPage() !== 'servers') return;

    const hasServerCards = qq('.kn-server-card').length > 0 || qq('a[href*="/server/"]').some(a => {
      if (a.closest('.kn-topbar, .kn-sidebar, nav, header, aside')) return false;
      try { return /^\/server\/[^/]+$/.test(new URL(a.href, window.location.origin).pathname); }
      catch (_) { return false; }
    });
    if (hasServerCards) return;

    const text = (document.body.textContent || '').toLowerCase();
    const looksEmpty = text.includes('no servers associated') || text.includes('tidak ada server') || text.includes('showing your servers');
    if (!looksEmpty) return;

    const card = el('div', 'kn-admin-empty-card');
    card.innerHTML = `
      <div class="kn-admin-empty-icon">${ICONS.admin}</div>
      <div class="kn-admin-empty-title">Admin Panel</div>
      <div class="kn-admin-empty-sub">
        Akun admin memang bisa saja tidak punya server pribadi. Untuk fitur owner/admin, buka menu Admin Panel.
      </div>
      <div class="kn-admin-empty-actions">
        <a href="/admin" class="kn-admin-btn">Buka Admin Panel</a>
        <a href="/admin/servers/new" class="kn-admin-btn secondary">Create Server</a>
      </div>
    `;
  ...
  function init() {
    ...
    injectTopbar(page);
    if (page !== 'login') document.body.classList.add('kn-has-topbar');
  ...
  const observer = new MutationObserver(() => {
    ...
    rafId = requestAnimationFrame(() => {
      rebrand(document.body);
      const page = getPage();
      if (page === 'login')               { injectLogin(); return; }
      injectTopbar(page);
      document.body.classList.add('kn-has-topbar');
      if (page === 'servers')             { upgradeServerCards(); wrapServerGrid(); ensureAdminAccessCard(); }
...
ENDJS
  ok "JS Nebula v9 ditulis."
}

apply_nebula_theme() {
  [[ -f "$PANEL_DIR/artisan" ]] || die "Panel belum valid di $PANEL_DIR. Install panel dulu."

  info "Apply Nebula Pro v9 Theme (full custom UI)..."
  write_nebula_css
  write_nebula_js

  # Inject ke semua layout blade yang punya <head>/<body>.
  # Ini penting karena halaman login/auth Pterodactyl biasanya tidak selalu pakai wrapper yang sama.
  python3 - "$PANEL_DIR" << 'PY'
import sys, pathlib, re, shutil
root = pathlib.Path(sys.argv[1])
views = root / "resources" / "views"
if not views.exists():
    raise SystemExit("Folder resources/views tidak ditemukan")

insert = """
    <!-- KAHFITZY_NEBULA_THEME_START -->
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link rel="stylesheet" href="/kahfitzy-nebula/nebula.css?v=9.0.0">
    <script defer src="/kahfitzy-nebula/nebula.js?v=9.0.0"></script>
...
for path in views.rglob('*.blade.php'):
    ...
    if '</head>' not in text and '</body>' not in text:
        continue
    new = text
    for pat in patterns:
        new = re.sub(pat, '\n', new, flags=re.S)
    if '</head>' in new:
        new = new.replace('</head>', insert + '</head>', 1)
    else:
        new = new.replace('</body>', insert + '</body>', 1)
    if new != text:
        backup = path.with_suffix(path.suffix + '.backup-kn-v8')
...
if not changed:
    raise SystemExit('Tidak ada blade layout yang bisa diinjeksi')
print('Inject theme berhasil ke %d file:' % len(changed))
for item in changed:
    print(' -', item)
PY
  ...
  ok "✅ Nebula Pro v7 berhasil! Tampilan login dan server manager BEDA TOTAL."
  echo ""
  info "Yang berubah di v7:"
...
install_panel_custom() {
  ...
  echo ""
  ok "╔══════════════════════════════════════════╗"
  ok "  INSTALL SELESAI — KAHFITZY NEBULA PRO v8"
  ok "╚══════════════════════════════════════════╝"
  echo -e "${GREEN}URL Panel :${NC} $(grep -E '^APP_URL=' "$PANEL_DIR/.env" 2>/dev/null | cut -d= -f2- | tr -d '"' || echo "http://${PANEL_DOMAIN}")"
...
menu() {
  while true; do
    banner
    echo "1. Install Panel + Nebula Pro v7 (Full Custom UI)"
    echo "2. Install Wings"
    echo "3. Install Panel + Wings + Theme"
    echo "4. Re-Apply Nebula Pro v9 Theme"
    echo "5. Repair Panel"
